...
try {
    $prod_id='41551PFXF3K14F91D8FABB';
    ...
    if ($data) {
        ...
        $pageTitle = 'Ice Walker GOT';
        $metaKeywords = 'Ice Walker GOT';
        $metaDescription = 'Ice Walker GOT';
    }
} catch (Exception $e) {
...
$canonicalURL = $link."ice-walker-got-collection";
...
$prod_id = '41551PFXF3K14F91D8FABB'; // your current product id
...
$prod_id = '41551PFXF3K14F91D8FABB'; // example product id
...
$prod_id = '41551PFXF3K14F91D8FABB'; // example product_id
...
<?php
// Note: error reporting and db.php are likely already included.
...
// The specific product ID for this page ("Ice Walker GOT")
$productId = '41551PFXF3K14F91D8FABB';
$data = null;
...
if (isset($data['data']) && is_array($data['data'])) {
    foreach ($data['data'] as $product) {
        if (isset($product['productCode']) && $product['productCode'] === $productId) {
            $wanted = $product;
            break;
        }
    }
...
if (!$wanted) {
    ...
    echo "</div>";
    // We exit here to prevent the booking form from rendering with incorrect data.
...
$productCode = safe_html($wanted['productCode'] ?? '');

// Price from description (fallback)
$desc = trim($wanted['description'] ?? '');
$lines = preg_split('/\r\n|\r|\n/', strip_tags($desc));
$priceRaw = isset($lines[3]) ? preg_replace('/^.*?:\s*/', '', trim($lines[3])) : '';
$price = '';
if ($priceRaw) {
    preg_match_all('/\$\d+/', $priceRaw, $matches);
    if (!empty($matches[0])) {
...
$minGuests = 2; // A sensible default
$maxGuests = 10; // A sensible default
if (isset($wanted['bookingLimits']['people']['min'])) {
    $minGuests = (int)$wanted['bookingLimits']['people']['min'];
}
if (isset($wanted['bookingLimits']['people']['max'])) {
    $maxGuests = (int)$wanted['bookingLimits']['people']['max'];
}
?>
...
<section>
    <div class="container">
        <div class="escape_room_home_Customer_Reviews_summary">
            ...
            // Product / category ID
            $prod_id = '41551PFXF3K14F91D8FABB'; // Replace dynamically if needed
...
<section>
    <div class="escape_room_home_contact_section">
        <div class="container">
            <div class="Boo_Prison_Escap_main_heading">
                <h2>Ready for ice Walker got?</h2>
            </div>
           <div class="row">
            ...
            <a href="#Book_one_single_pr" class="bg_bnt_custom  continue_nex_step"> Book Ice Walker Now</a>
        </div>
    </div>
</section>

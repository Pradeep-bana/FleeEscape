...
// ---------------------------
$selected_page = 'christmas-new-year';
...
$canonicalURL = $link."christmas-new-year-escape-room-vr-party";
$videoModalTitle = 'Chritsmas New Year';
...
$stmt->execute([':page' => 'christmas-new-year']);
$pageData = $stmt->fetch(PDO::FETCH_ASSOC);
...
$selectedPage = 'christmas-new-year'; // can be dynamic as needed
...
// Fetch data for selected page
$selectedPage = 'christmas-new-year'; // change dynamically if needed
...
<section class="party-packages" id="BuildingPackage">
    <div class="container">
        <div class="section_heading_page">
...
<?php include("party-package-all.php"); ?>
...
// Fetch data for selected page
$selectedPage = 'christmas-new-year'; // set dynamically if needed
...
$selectedPage = 'christmas-new-year'; // or dynamic based on current page
...
// Fetch data for selected page
$selectedPage = 'christmas-new-year';
$stmt = $pdo->prepare("SELECT 
...
// Define selected page
$selectedPage = 'christmas-new-year'; // Replace dynamically if needed
...
<div class="faq-section fade-in-up" style="animation-delay: 0.6s;">
    ...
    <form id="serviceEnquiryForm" data-aos="fade-up" data-aos-delay="200">
    <div class="form-group">
        <input id="enq_fullname" name="fullname" type="text" placeholder="Full Name *" required>
        <input id="enq_email" name="email" type="email" placeholder="Email Address *" required>
    </div>

    <div class="form-group">
        <input id="enq_phone" name="phone" type="number" placeholder="Phone Number *" required>
        <input id="enq_company" name="company" type="text" placeholder="Company Name">
    </div>

    <div class="form-group">
        <select id="enq_service" name="service" required>
        ...
        <input id="enq_date" name="event_date" type="date" required>
    </div>

    <textarea id="enq_message" name="message" placeholder="Your Message *" required></textarea>
...
document.getElementById("serviceEnquiryForm").addEventListener("submit", function(e){
    ...
    let fullname = document.getElementById("enq_fullname").value.trim();
    let email = document.getElementById("enq_email").value.trim();
    let phone = document.getElementById("enq_phone").value.trim();
    let company = document.getElementById("enq_company").value.trim();
    let service = document.getElementById("enq_service").value;
    let event_date = document.getElementById("enq_date").value;
    ...
    if (fullname.length < 3) {
        responseBox.innerHTML = "<span style='color:red'>Please enter your full name.</span>";
        return;
    }

    let emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailPattern.test(email)) {
        responseBox.innerHTML = "<span style='color:red'>Please enter a valid email address.</span>";
        return;
    }

    if (!/^[0-9]{10,15}$/.test(phone)) {
        responseBox.innerHTML = "<span style='color:red'>Enter a valid phone number (10-15 digits).</span>";
        return;
    }

    if (service === "") {
        responseBox.innerHTML = "<span style='color:red'>Please select a service.</span>";
        return;
    }

    if (event_date === "") {
        responseBox.innerHTML = "<span style='color:red'>Please select a date.</span>";
        return;
    }

    if (message.length < 5) {
        responseBox.innerHTML = "<span style='color:red'>Please enter your message.</span>";
        return;
    }
    ...
    fetch("send_service_enquiry.php", {
        method: "POST",
        body: formData
    })
    .then(res => res.json())
    ...
    .catch(err => {
        responseBox.innerHTML = "<span style='color:red'>Something went wrong. Try again later.</span>";
    });
});
</script>

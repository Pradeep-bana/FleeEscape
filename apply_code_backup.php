...
foreach ($cartItems as $item) {
    ...
    if ($res['code'] === 201) {
        ...
        // We delete this hold immediately because we need to re-book if we add vouchers.
        // If we don't add vouchers, we'll recreate it at the end.
        deleteBookeoHold($data['id'], $apiKey, $secretKey);
        
        // Loop through Voucher Pool (Greedy Consumption)
        foreach ($voucherPool as $key => $code) {
            if ($netPrice <= 0) break; // Stop if fully paid
            ...
            if ($vRes['code'] === 201) {
                ...
                // Check if this specific code actually added value
                if ($newCredit > $currentCredit) {
                    // It worked! Keep it.
                    $appliedVouchersForThisItem[] = $code;
                    $currentCredit = $newCredit;
                    $netPrice = $vData['price']['totalNet']['amount'];

...
if ($_SERVER['REQUEST_METHOD'] === 'POST'
    ...
    foreach ($_SESSION['cart'] as $item) {
        ...
        if ($item['eventId'] === $eventId && $existingDate === $slotDate) {
            echo json_encode([
                'status' => 'error',
                'message' => 'This slog is already in your cart for the same date.',
                'cart' => $_SESSION['cart']
            ]);
            exit;
        }
    ...
    // Wiping product+date → slot_count = 0 → 'missing' → blocking re-fetch → instant correct display.
    $slotDateForCache = isset($slot) ? substr($slot, 0, 10) : null;

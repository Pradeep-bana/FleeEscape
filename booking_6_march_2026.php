                ...
                success: function(response) {
                    ...
                    productIds.forEach(productId => {
                        ...
                        if (html.includes("No slots available") && allowAutoNextDay && allowNextDayByTime) {
                            const $input = $(`.custom-datepicker_input[data-product="${productId}"]`);
                            if ($input.length) {
                                const currentRequestedDate = DateTime.fromISO(rawDate, { zone: LA_ZONE });
                                const nextDay = currentRequestedDate.plus({ days: 1 });
                                const diffFromToday = nextDay.diff(today, 'days').days;
                                if (diffFromToday <= 1) {
                                    const nextRaw = nextDay.toFormat('yyyy-MM-dd');
                                    $input.data('rawdate', nextRaw);
                                    $input.val(fmtDisplay(nextDay));
                                    console.log(`Auto-advance: No slots for ${productId} on ${rawDate}, trying ${nextRaw}`);
                                    fetchSlotsForProducts([productId], nextRaw, false); 
                                } else {
                                    console.log(`Auto-advance stopped: ${productId} has no slots on ${rawDate}, and we are not auto-advancing further.`);
                                }
                            }
                        }
                    });
